...
DeskWrap 是**单个 Go 可执行文件（约 8.5 MB）**，借助系统 [WebView2 运行时](https://developer.microsoft.com/microsoft-edge/webview2/)（Windows 11 自带）把 `localhost` 的 Web 服务变成真正的桌面窗口。不捆绑 Electron、不捆绑 Node，`deskwrap build` 秒级完成，产物只有 **5–8 MB 的 zip**。

AI 生成的项目缺的"最后一公里"：代码有了，但装依赖、启动、变成双击就能用的应用，还要在终端里折腾。DeskWrap 替你完成——服务起不来时，内置 AI 分析日志定位原因，智能体自动修复（装依赖、换端口、调配置）。

<div align="center">
  <img src="docs/screenshots/panel-project.png" width="480" alt="项目面板"/>
  <img src="docs/screenshots/panel-log.png" width="480" alt="日志与 AI 诊断面板"/>
</div>
...
- 🟢 **环境管理** — 用**你自己的**工具链（Node/npm/pnpm/yarn/Git/Python，支持 nvm/volta/fnm），不捆绑任何运行时
- 🤖 **AI 诊断** — 24 家提供商；分析日志、定位根因、给出修复建议（Key 仅存本地）
- 🛠️ **智能体** — "让项目跑起来"：自动装依赖、修端口/配置，最多 15 轮，危险命令拦截
- 🌐 **一键开源** — 搜索 GitHub / Gitee / HuggingFace / 魔搭 / GitLab / Codeberg，一键克隆部署
- 📦 **极小产物** — `build` = 复制 exe + 配置 + WebView2Loader.dll → zip，秒级完成
...
deskwrap build ./my-project  # 打包（exe + 配置 + dll → zip）
...
| 场景 | 其他工具 | DeskWrap |
| --- | --- | --- |
| AI 生成的项目跑不起来 | 靠 agent / 手动修 | **AI 日志诊断 + 智能体自动修复**（依赖、端口、配置） |
| 打包体积/时间 | Electron 250MB+ / Tauri 编译分钟级 | **5–8MB zip，秒级** |
| 工具链要求 | Tauri 要 Rust/Cargo、Electron 要 Node | **零工具链**：单 Go exe + 系统 WebView2 |
| 面向对象 | 成品网站/静态 URL（Pake、nativefier） | **开发中的服务**——自动检测命令/端口、管进程树 |
| 试用开源项目 | 克隆 → 自己配 → 手动起 | **一键**：搜索 → 克隆 → 检测 → 跑起来 → 打包 |

DeskWrap 与 AI agent（opencode / claude-code 类）是**互补**关系：它们负责**写**代码，DeskWrap 负责**落地**——装、跑、打包。Roadmap 已规划 MCP/CLI 接口打通这条闭环。
...
见 [ROADMAP.md](ROADMAP.md)。护城河 = **自动检测 + 进程树治理 + AI 修复闭环**，随后是 agent 生态集成（MCP/CLI）、项目模板、macOS/Linux 原生窗口。
