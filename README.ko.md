...
DeskWrap은 **단일 Go 실행 파일(약 8.5 MB)** 입니다. 시스템의 [WebView2 런타임](https://developer.microsoft.com/microsoft-edge/webview2/)(Windows 11 기본 내장)을 사용해 `localhost` 웹 서비스를 진짜 데스크톱 창으로 바꿔 줍니다. Electron도, 250MB급 결과물도 없습니다. `deskwrap build`는 프로젝트를 의존성과 함께 **번들된 Node.js 런타임**까지 패키징합니다. 받는 쪽은 **압축 풀고 더블클릭만 하면 됩니다** — Node.js / Python / 브라우저 설치가 전혀 필요 없습니다.

AI가 생성한 프로젝트에 빠져 있던 "마지막 1마일": 코드는 있는데 의존성 설치·실행·더블클릭 앱화에는 여전히 터미널 작업이 필요합니다. DeskWrap이 대신합니다 — 서비스가 안 뜨면 내장 AI가 로그를 분석해 원인을 찾고 수정 방법을 제시합니다.
...
- 🟢 **환경 관리자** — 개발·디버깅 시 **내가 설치한** 툴체인 사용(Node/npm/pnpm/yarn/Git/Python, nvm/volta/fnm 지원); 패키징된 앱에는 Node.js 번들 포함
...
- 📦 **더블클릭 배포** — `build`가 프로젝트 + 의존성 + Node.js 런타임을 번들. 받는 쪽 설치 불필요. 비밀 정보(`deskwrap.config.json`·`.env`·프록시/AI 설정)는 결과물에서 자동 제거
...
3. 또는 **📦 빌드**로 독립 실행 앱 zip 생성(Node.js 포함) — 상대방은 압축 풀고 더블클릭만
...
deskwrap build ./my-project  # 패키징(exe + 설정 + app + 번들 Node → zip)
...
| 상황 | 다른 도구 | DeskWrap |
| --- | --- | --- |
| AI 생성 프로젝트가 안 돌아감 | 에이전트에 의존 / 수동 수정 | **AI 로그 진단**(원인 파악 + 수정 제안) |
| 패키지 크기 / 빌드 시간 | Electron 250MB+ / Tauri 수 분 컴파일 | **도구 본체 3.6MB zip; 전체 패키지 ≈ 프로젝트 크기(70–100MB), 1–3분**(복사만, 컴파일 없음) |
| 필요한 툴체인 | Tauri는 Rust/Cargo, Electron은 Node | **받는 쪽은 제로** — Go exe 하나 + 시스템 WebView2, Node는 패키지에 번들 |
| 대상 | 완성 사이트 / 정적 URL(Pake, nativefier) | **개발 중인 서비스** — 명령/포트 자동 감지, 프로세스 트리 관리 |
| 오픈소스 프로젝트 체험 | 클론 → 직접 설정 → 수동 실행 | **원클릭**: 검색 → 클론 → 감지 → 실행 → 패키징 |

DeskWrap은 AI 에이전트(opencode / claude-code 계열)와 **보완 관계**입니다: 에이전트가 코드를 **쓰고**, DeskWrap이 **착륙**시킵니다(설치·실행·패키징).
...
[ROADMAP.md](ROADMAP.md) 참조. 핵심 경쟁력은 **자동 감지 + 프로세스 트리 관리 + AI 로그 진단**, 이후 템플릿, macOS/Linux 네이티브 창.
